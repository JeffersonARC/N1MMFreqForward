...
def show_settings_dialog(root: tk.Tk, cfg: dict, on_saved: Callable[[dict], None]) -> None:
    ...
    dlg.title("N1MM Frequency Forwarder settings")
    dlg.transient(root)
    ...
    def on_ok():
        try:
            ...
            messagebox.showerror("N1MM Frequency Forwarder", "Send interval must be an integer.")
            return
        new_cfg = dict(cfg)
...
def main() -> int:
    setup_logging()
    logging.info("N1MM Frequency Forwarder starting; config=%s", CONFIG_FILE)
    ...
    try:
        ...
        logging.info("N1MM Frequency Forwarder shutting down")
        try: icon.stop()
        except Exception: pass
        relay.stop()
    return 0
